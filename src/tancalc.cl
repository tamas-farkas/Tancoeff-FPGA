...
/*
static void ref_read0(__global uint16 *dataset1_0, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
...
static void ref_read1(__global uint16 *dataset1_1, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
	__attribute__((xcl_pipeline_loop(1)))
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
static void ref_read2(__global uint16 *dataset1_2, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
	ushort16 temp;
	__attribute__((xcl_pipeline_loop(1)))
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
...
static void ref_read3(__global uint16 *dataset1_3, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
	__attribute__((xcl_pipeline_loop(1)))
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
static void ref_popcount0(__global uint16 *dataset1_0, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local){
	ushort16 temp;
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
static void ref_popcount1(__global uint16 *dataset1_1, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local){
	ushort16 temp;
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
static void ref_popcount2(__global uint16 *dataset1_2, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local){
	ushort16 temp;
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
static void ref_popcount3(__global uint16 *dataset1_3, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local){
	ushort16 temp;
	__attribute__((xcl_loop_tripcount(c_size, c_size)))
	for (ushort ref_num = 0; ref_num < c_size; ref_num++){
	if (ref_num % 2 == 0) {
...
void ref_read(__global uint16 *dataset1_0, __global uint16 *dataset1_1, __global uint16 *dataset1_2, __global uint16 *dataset1_3, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
	...
	ref_popcount0(dataset1_0, ref_local, refpop_local);
	ref_popcount1(dataset1_1, ref_local, refpop_local);
	ref_popcount2(dataset1_2, ref_local, refpop_local);
	ref_popcount3(dataset1_3, ref_local, refpop_local);
}*/
void ref_read(__global uint16 *dataset1_0, __global uint16 *dataset1_1, __global uint16 *dataset1_2, __global uint16 *dataset1_3, __local uint16 ref_local[LOCAL_MEM_SIZE][2], __local ushort *refpop_local, ushort chunk_num){
	ushort16 temp;
	__attribute__((xcl_pipeline_loop(1)))
