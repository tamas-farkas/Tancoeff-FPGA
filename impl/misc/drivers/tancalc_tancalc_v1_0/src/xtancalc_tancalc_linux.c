...
typedef struct {
    ...
    XTancalc_tancalc_uio_map maps[ MAX_UIO_MAPS ];
} XTancalc_tancalc_uio_info;

/***************** Variable Definitions **************************************/
static XTancalc_tancalc_uio_info uio_info;
...
static int uio_info_read_name(XTancalc_tancalc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTancalc_tancalc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTancalc_tancalc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTancalc_tancalc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTancalc_tancalc_Initialize(XTancalc_tancalc *InstancePtr, const char* InstanceName) {
	XTancalc_tancalc_uio_info *InfoPtr = &uio_info;
