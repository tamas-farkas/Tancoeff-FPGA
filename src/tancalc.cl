...
void ref_read(__global const uint16 *dataset1_0, __global const uint16 *dataset1_1, __global const uint16 *dataset1_2, __global const uint16 *dataset1_3,  uint16 ref_local[LOCAL_MEM_SIZE][2],  ushort *refpop_local, ushort chunk_num){
	...
	for(uchar i = 0; i < LOCAL_MEM_SIZE; i++){
		refpop_local[i] = 0;
	}
...
void calculation( uint16 ref_local[LOCAL_MEM_SIZE][2],  uint16 cmpr_local[4][2],  ushort *refpop_local,  ushort *cmprpop_local, uchar result_local[LOCAL_MEM_SIZE][4]){
	...
	calculation_c1: for(uchar n = 0; n < 4; n++){
		__attribute__((xcl_loop_tripcount(LOCAL_MEM_SIZE, LOCAL_MEM_SIZE)))
		calculation_c2: for(ushort ref_num = 0; ref_num < LOCAL_MEM_SIZE; ref_num++){
			ushort16 temp;
			ushort temp2 = 0;
...
void tancalc(__global const uint16 *dataset1_0, __global const uint16 *dataset1_1, __global const uint16 *dataset1_2, __global const uint16 *dataset1_3,
	__global const uint16 *dataset2_0, __global const uint16 *dataset2_1, __global const uint16 *dataset2_2, __global const uint16 *dataset2_3,
